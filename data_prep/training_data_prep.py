...
def get_yellow_card_data(event_df):
    ...
    df3 = df3["opp_yellow_cards"] - df3["yellow_cards"]
...
if __name__ == "__main__":
    ...
    for match_id in match_ids[:20]:
        print(match_id)
        if match_id not in [3754136,3754045,3754053,3754226,3754042,3754058]:
            ...
            merged_df = goals.merge(yellow_card, on=["match_id","team_id","team","minute"], how='inner')  # Merge df1 and df2
            merged_df = merged_df.merge(shots_generated, on = ["match_id","team_id","team","minute"], how = 'inner')
            merged_df = merged_df.merge(red_card, on=["match_id","team_id","team","minute"], how='inner')  # Merge with df3
    ...
    minute_df = data.groupby("match_id").agg({"minute":"max"}).reset_index().rename(columns = {"minute":"max_minute_game"})
    ...
    temp_data_df = pd.merge(data, matches_df[["match_id","home_team","away_team","home_score","away_score"]], on = 'match_id')
    temp_data_df["final_goals"] = np.where(temp_data_df["team"] == temp_data_df["home_team"],temp_data_df["home_score"],temp_data_df["away_score"])
    data = temp_data_df[data_columns]
    
    data.to_csv(r"C:\Users\adams\Documents\Courses\Monte_Carlo_Methods\Live_Prediction_Football_Model\data_files\soccer_training_data.csv")